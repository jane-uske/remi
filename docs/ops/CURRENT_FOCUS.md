...
**结构性止血（SD-01~07）已全部完成**（2026-06-11 按实际代码核对，状态见 [TASKS.md](./TASKS.md)），当前最高优先级回到体验主线：**Web 默认入口 10 分钟在场感体验（W-PRES-01~04）**。

终局方向已固化为 [REMIWORLD_NORTH_STAR.md](../design/REMIWORLD_NORTH_STAR.md)（虚拟世界伴侣）：
**世界是这条在场感主线的空间化载体，不是第二条主线**；世界线（RW-P1，见 TASKS.md 并行支线）吃 W-PRES 的大脑侧产出，不与其抢资源。

补充事实（2026-06-11）：SD-05 的情绪自标注此前因流式跨 chunk 解析 bug 实际从未生效（标签泄漏给客户端、avatar/TTS 情绪一直走关键词兜底），已修复并经真实链路验证。也就是说，**表现层第一次拿到了正确的情绪信号**，W-PRES-03（在场感统一）从现在起才真正具备开工条件。
...
终极目标的具体形态已固化在 [REMIWORLD_NORTH_STAR.md](../design/REMIWORLD_NORTH_STAR.md)：
LLM 驱动的虚拟世界伴侣——她生活在自己的世界里，用户进入时她从正在做的事情中抬头。
三条关系判断：

1. **世界 = 在场感的终局载体**：W-PRES-03 收口的语音/口型/表情/turn state，就是世界 Phase 1 的直接输入
2. **大脑侧修复（W-PRES-01/02）世界自动受益**：世界线不重复做、不抢资源
3. **Memory V2 读路径以世界为真实消费方**："她结合记忆回应"是读路径最自然的验证场

世界基建现状：RemiWorld v0.1 已可玩（`/world`：小岛/房间/记忆墙/剧本任务，像素纹理+AO+bloom 日落渲染），后续按 NORTH_STAR 的 RW-P1 阶段推进。
