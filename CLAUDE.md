...
建设期结束（活人感+记忆时效两轮大改造已部署，见 `docs/ops/STATE_OF_THE_PROJECT_2026-07.md`）。当前主线：

1. **真实使用**是唯一北极星（每周真实聊天天数/轮数）；坏样本 → 修复 → 固化为探针（`scripts/memory_probe_eval.ts`，已 7 根）
2. **收缩**：执行「最小可爱 Remi」——iOS/watchOS/desktop/STT/视频生成等为**冻结区**，不为其写代码、不修其测试
3. 改记忆/prompt 前后必跑探针与 chat_vitality_eval（防退化红线）

执行看板在 `docs/ops/TASKS.md`（2026-07-03 篇章）。
